...
# Database Setup Script for Flair Social
# This script installs database dependencies when you're ready to migrate
...
# Add database scripts to package.json if they don't exist
node -e "
const fs = require('fs');
const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));
pkg.scripts = pkg.scripts || {};
pkg.scripts['db:generate'] = 'drizzle-kit generate:pg';
pkg.scripts['db:push'] = 'drizzle-kit push:pg';
pkg.scripts['db:studio'] = 'drizzle-kit studio';
pkg.scripts['db:setup'] = 'npm run db:generate && npm run db:push';
pkg.scripts['db:migrate'] = 'node -e \"console.log(\\\"Migration script would run here\\\")\"';
fs.writeFileSync('package.json', JSON.stringify(pkg, null, 2));
console.log('✅ Database scripts added to package.json');
"

echo ""
echo "📋 Current performance status:"
curl -s http://localhost:3000/api/health | json_pp || echo "Start your dev server to see performance metrics"
